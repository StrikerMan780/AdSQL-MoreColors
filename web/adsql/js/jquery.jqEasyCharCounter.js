...
$.fn.extend({
    jqEasyCounter: function(givenOptions) {
        return this.each(function() {
			...
			var colors = ["{DEFAULT}","{LIGHTGREEN}", "{TEAM}", "{GREEN}", "{OLIVE}", "{WHITE}",       "{RED}",        "{GREEN}",   "{BLUE}",    "{YELLOW}",    "{PURPLE}",    "{CYAN}",      "{ORANGE}",    "{PINK}",      "{OLIVE}",     "{LIME}",      "{VIOLET}",    "{LIGHTBLUE}", "\\n"];
			function cut(str, cutStart, cutEnd){
				  return str.substr(0,cutStart) + str.substr(cutEnd+1);
			}
			function doCount(){
				var val = $this.val(),
					length = val.length, alength = 0, aval = $this.val();
				for(var i = 0; i < colors.length; i++) {
					while (aval.indexOf(colors[i]) !== -1) {
						alength = alength + 2;
						aval = cut(aval, aval.indexOf(colors[i]), aval.indexOf(colors[i])+colors[i].length)
					}
				}
				...
				alength = alength + aval.length;
				if(alength >= options.maxCharsWarning){
				...
				if (options.nocomment) {
					...
					jqEasyCounterMsg.html('Characters: ' + $this.val().length + "/" + options.maxChars + " (length in game:" + alength + " (max:"+options.maxCharsWarning+")) Warning! Max Length depends on how wide - are used characters, 190 refer to '1'");
				}
